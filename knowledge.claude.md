...
| `504 Gateway Timeout` (`unexpected end of JSON input`) on enrich requests | NIM gateway gives up waiting for the 70B to finish a long/verbose generation (known issue on the hosted endpoint); retries don't help if a batch consistently exceeds the window | **Stream** the completion (`CreateChatCompletionStream`, accumulate `Delta.Content`) — continuous SSE keeps the connection alive so the gateway never times out; the per-request `ctx` deadline becomes the real bound |
